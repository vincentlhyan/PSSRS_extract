...
public class ChinaSellIn_all_w_service_inbilling_grid_pricing_v3 {
	...
	public static void main(String[] args) throws Exception {
		...
		mapOfArgs.put("DB.PASSWORD", "bhu8bhu8");
		//full sell-in
		mapOfArgs.put("PATCH", "XTRANSTYPE, +BILL-, MICXT");
		mapOfArgs.put("CACHEFILE", "SELLIN.cache");
		mapOfArgs.put("STARTTACTIONTS", "2016-01-01 00:00:00.000000");
		mapOfArgs.put("ENDTACTIONTS", "2017-06-08 00:00:00.000000");
		mapOfArgs.put("COUNTER", "0");
		
		(new ChinaSellIn_all_w_service_inbilling_grid_pricing_v3()).createReport(mapOfArgs);
...
//Invoice data, if order reason = 'BLB'...	
		...
		//"AND (T2BILL.ORDER_REASON IN (" + sOrderReason + ")) " +
		//"AND (T2BILL.LEGAL_CONTRCT_CPS_NO IN ('8300138627',	'8300144658',	'8300144833',	'8300006881',	'8300144810',	'8300121850',	'8300081480',	'8300060523',	'8300007204',	'8300081480',	'8300060523',	'8300060523')) " +
		...
		sQuery = "SELECT DISTINCT HDER.SALES_DOC_ORDER_NO, "
                ...
                "AND (HDER.SALES_DOC_TYPE IN ('ZGOR','ZBPS','ZNPC','ZPBC')) " +
                ...
                //"AND (HDER.SALES_DOC_ORDER_NO = '7002770446') " +
				"FOR READ ONLY "; 
		...
		while (rs.next()) {
	    	...
	    	if ((ar_sField[3]!=null) && ("ZNPC".equals(ar_sField[3])|| "ZPBC".equals(ar_sField[3]) )) {
...
/*			sQuery = "SELECT RTRIM(CRM.QUOTE_DOC_NO) AS QUOTE_DOC_NO, RTRIM(CRM.QUOTE_DOC_ITEM_NO) AS QUOTE_DOC_ITEM_NO, RTRIM(CRM.PO_NUMBER_SOLD) AS PO_NUMBER_SOLD " +
		...
		ar_sField = new String[] {"COUNTRY_CODE",
				...
				"PROMO_ID", "INVOICE_UNIT_AMOUNT","LIST_UNIT_PRICE",
				"BID_FLAG", "RR_FLAG", 
				...
				"CFT_NO","CFT_NAME","CFT_CEI","CFT_ENAME","CFT_CMR"}; 
		...
		final int LIST_UNIT_PRICE = INVOICE_UNIT_AMOUNT + 1;
		final int BID_FLAG = LIST_UNIT_PRICE + 1;
		...
		for (int s=0; s<listOfInvoice.size(); s++) {
			try {
				...
				ar_sField[LIST_UNIT_PRICE] = null;
...
*/
				...
				ar_sField[LIST_UNIT_PRICE]=listOfInvoice.get(s)[26];
                                    ...
                                    if  (ar_sField[MIC].equals("X"))  {
                                    	if (listOfFeatures.get(iSub)[4].startsWith("ECS0")){
        									...
        									for (int r=0; r<m_nfUS.parse(listOfFeatures.get(iSub)[5]).intValue(); r++) {
        										...
        										ar_sField[LIST_UNIT_PRICE] = String.valueOf("0");
        										ar_sField[UNIQUEREF]=sNow + formatCount();							
        										addRow(ar_sField);
        									}
                                    	}
    									...
    									for (int r=0; r<p; r++) {
    										try {
												...
												ar_sField[LIST_UNIT_PRICE] = String.valueOf(Double.parseDouble(ar_sField[LIST_UNIT_PRICE]) / Double.parseDouble(listOfFeatures.get(iSub)[5]));
											} catch (Exception nfe) {
												ar_sField[INVOICE_UNIT_AMOUNT] = null;
											}
    										ar_sField[UNIQUEREF]=sNow + formatCount();							
